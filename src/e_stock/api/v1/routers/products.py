...
from e_stock.schemas.products import ProductOut, ProductBase, ProductPatch
...
async def get_product_by_id(id: UUID, session: AsyncSession = Depends(get_db_session)):
    prod_repo = ProductRepository(session)
    product = await prod_repo.get_by_id(id)
    if product:
        return product
    raise ProductNotFound(id)

@router.post('/', response_model=ProductOut)
async def create_product(product: ProductBase, session: AsyncSession = Depends(get_db_session)):
    prod_repo = ProductRepository(session)
    return await prod_repo.add(product)

@router.put('/{id}', response_model=ProductOut)
async def update_product(id: UUID, product: ProductBase, session: AsyncSession = Depends(get_db_session)):
    prod_repo = ProductRepository(session)
    return await prod_repo.update(id, product)
...
async def patch_product(id: UUID, product: ProductPatch, session: AsyncSession = Depends(get_db_session)):
    prod_repo = ProductRepository(session)
    return await prod_repo.patch(id, product)
